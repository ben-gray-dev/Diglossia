...
class Path {
    ...
    construct(segments, offset, length) {
        const path = Object.create(Object.getPrototypeOf(this));
...
class AsyncQueue {
    ...
    enqueueAfterDelay(timerId, delayMs, op) {
        ...
        const delayedOp = DelayedOperation.createAndSchedule(this, timerId, delayMs, op, op => this.removeDelayedOperation(op));
...
class Document extends MaybeDocument {
    constructor(key, version, data, options, 
    /**
...
class IndexedDbRemoteDocumentCache {
     ...
     */
    addEntries(transaction, entries, sizeDelta) {
        const promises = [];
        if (entries.length > 0) {
...
class IndexedDbPersistence {
    ...
    constructor(persistenceKey, clientId, platform, queue, serializer, lruParams, multiClientParams) {
        this.persistenceKey = persistenceKey;
        this.clientId = clientId;
        this.queue = queue;
    ...
    static async createIndexedDbPersistence(persistenceKey, clientId, platform, queue, serializer, lruParams) {
        const persistence = new IndexedDbPersistence(persistenceKey, clientId, platform, queue, serializer, lruParams);
        await persistence.start();
        return persistence;
    }
    static async createMultiClientIndexedDbPersistence(persistenceKey, clientId, platform, queue, serializer, lruParams, multiClientParams) {
        const persistence = new IndexedDbPersistence(persistenceKey, clientId, platform, queue, serializer, lruParams, multiClientParams);
        await persistence.start();
        return persistence;
    }
    /**
...
class IndexedDbLruDelegate {
     ...
     * its sentinel row.
     */
    removeOrphanedDocument(txn, docKey) {
            ...
            if (!remoteVersion.isEqual(SnapshotVersion.MIN)) {
                const updateRemoteVersion = this.queryCache
                    ...
                    assert(remoteVersion.compareTo(lastRemoteVersion) >= 0, 'Watch stream reverted to previous snapshot?? ' +
                        ...
                        ' < ' +
                        lastRemoteVersion);
                    return this.queryCache.setTargetsMetadata(txn, txn.currentSequenceNumber, remoteVersion);
                });
...
class MemoryRemoteDocumentChangeBuffer extends RemoteDocumentChangeBuffer {
    constructor(sizer, documentCache) {
        super();
        this.sizer = sizer;
        this.documentCache = documentCache;
    }
...
class SyncEngine {
     ...
     */
    async runTransaction(updateFunction, retries = 5) {
...
class FirestoreClient {
    ...
    startIndexedDbPersistence(user, settings) {
        ...
        const storagePrefix = IndexedDbPersistence.buildStoragePrefix(this.databaseInfo);
        ...
        return Promise.resolve().then(async () => {
            ...
            if (settings.synchronizeTabs) {
                ...
                persistence = await IndexedDbPersistence.createMultiClientIndexedDbPersistence(storagePrefix, this.clientId, this.platform, this.asyncQueue, serializer, lruParams, { sequenceNumberSyncer: this.sharedClientState });
            }
            else {
                this.sharedClientState = new MemorySharedClientState();
                persistence = await IndexedDbPersistence.createIndexedDbPersistence(storagePrefix, this.clientId, this.platform, this.asyncQueue, serializer, lruParams);
            }
            this.persistence = persistence;
